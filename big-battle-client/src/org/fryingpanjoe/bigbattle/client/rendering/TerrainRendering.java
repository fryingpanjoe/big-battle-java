...
public class TerrainRendering {
  ...
  public void renderTerrainPatch(final IsometricCamera camera, final TerrainPatch patch) {
    ...
    final float minX = -d;
    final float maxX =  d;
    final float minY = -2.f * SQRT_TWO * d;
    final float maxY =  2.f * SQRT_TWO * d;
    ...
    final int tileMinX = Math.max(0, (int)((cx + tileTL.x - 0.5f) / TILE_SIZE));
    final int tileMaxX = Math.min(patch.getSize(), (int)((cx + tileBR.x + 0.5f) / TILE_SIZE));
    final int tileMinY = Math.max(0, (int)((cy + tileTR.y - 0.5f) / TILE_SIZE));
    final int tileMaxY = Math.min(patch.getSize(), (int)((cy + tileBL.y + 0.5f) / TILE_SIZE));
